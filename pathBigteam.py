...
class PathFollower(Node):
    def __init__(self):
        ...
        # Parameters
        self.path_before = np.load('banana_path.npy')  # Load saved path
        self.path = self.path_before[:, [1, 0]]
        self.grid_map = np.load('banana.npy')  # Load grid map
        self.grid_size = 0.1  # Grid resolution in meters per cell
    ...
    def icp(self, source, target, max_iterations=100, tolerance=1e-5):
        """
